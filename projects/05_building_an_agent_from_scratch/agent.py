...
# The first thing we need is a model that will process 
# user input. We’ll create an OllamaModel class, 
# which interacts with a local API to generate responses.
...
# the ToolBox class stores all the tools the agent can use and
...
# The agent needs to think, decide which tool to use, 
# and execute it. 
# Here’s the Agent class:

agent_system_prompt_template = """
You are an intelligent AI assistant with access to specific tools.
Your response must ALWAYS be in the JSON format:
{{
...
2. reverse_string: Use for ANY request involving reversing text
    ...
    - Example inputs and outputs:
      Input: "Reverse of 'Howwwwww'?"
      Output: {{"tool_choice": "reverse_string", "tool_input": "Howwwwww"}}

      Input: "What is the reverse of Python?"
      Output: {{"tool_choice": "reverse_string", "tool_input": "Python"}}

3. no tool: Use for general conversations and questions
    - Example inputs and outputs:
      Input: "Who are you?"
...
if __name__ == "__main__":
    ...
    # using Ollama with llama3.2 model
    model_service = OllamaModel
    model_name = "llama3.2" # Can be changed to other models like 'mistral', 'codellama', etc.
    stop = "<|eot_id|>"
